...
public class SM4 {
    ...
    private int[] expandKey(boolean forEncryption, byte[] key) {
        ...
        MK[0] = bigEndianToInt(key, 0);
        MK[1] = bigEndianToInt(key, 4);
        MK[2] = bigEndianToInt(key, 8);
        MK[3] = bigEndianToInt(key, 12);
    ...
    public byte[] processBlock(byte[] in) {
        ...
        if (BLOCK_SIZE > in.length) {
            throw new IllegalArgumentException("input buffer too short");
        }

        X[0] = bigEndianToInt(in, 0);
        X[1] = bigEndianToInt(in, 4);
        X[2] = bigEndianToInt(in, 8);
        X[3] = bigEndianToInt(in, 12);
        ...
        intToBigEndian(X[3], out, 0);
        intToBigEndian(X[2], out, 4);
        intToBigEndian(X[1], out, 8);
        intToBigEndian(X[0], out, 12);
    ...
    private static int bigEndianToInt(byte[] var0, int var1) {
